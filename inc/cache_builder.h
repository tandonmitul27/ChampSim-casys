...
struct cache_builder_base {
  ...
  bool m_bypass{false};
  bool m_is_spm{false};
  bool m_bypass_mem_type_0{false};
...
public:
  ...
  /**
   * Mark this cache to bypass (not install or serve) mem_type==0 accesses.
   * Used in B2: L1D/L2C cache only important (SPM-tagged) data.
   */
  self_type& set_bypass_mem_type_0();
...
template <typename P, typename R>
auto champsim::cache_builder<P, R>::set_bypass_mem_type_0() -> self_type&
{
  m_bypass_mem_type_0 = true;
  return *this;
}
